...
feature_used_label = kmeans(feature_used, 3)

# 存储feature_used_label到.txt文件
np.savetxt('STCMN/param/struct/feature_used_label.txt', feature_used_label)

#%% 读取feature_used_label
feature_used_label = np.loadtxt('STCMN/param/struct/feature_used_label.txt')
...
class EarlyStopping:
    ...
    def save_checkpoint(self, val_loss, model, task_kind):
        ...
        torch.save(model.state_dict(), 'STCMN/param/struct/checkpoint_mul_'+str(task_kind)+'.pth')  # 这里会存储迄今最优模型的参数
...
#%% 读取模型参数
model0.load_state_dict(torch.load('STCMN/param/struct/checkpoint_mul_0.pth'))
model1.load_state_dict(torch.load('STCMN/param/struct/checkpoint_mul_1.pth'))
model2.load_state_dict(torch.load('STCMN/param/struct/checkpoint_mul_2.pth'))
